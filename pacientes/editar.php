...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        $stmt = $pdo->prepare("UPDATE pacientes SET nome = ?, cpf = ?, email = ?, telefone = ?, 
                              data_nascimento = ?, endereco = ? WHERE id = ?");
        $stmt->execute([$nome, $cpf, $email, $telefone, $data_nascimento, $endereco, $id]);
